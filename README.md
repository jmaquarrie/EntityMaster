...
6. The compact sidebar starts collapsed—use the **Filters & Search (+/−)** toggle to expand it. The panel now offers:
   ...
   - Click empty canvas space to reset the current filters/highlights.
...
8. Hover over any dotted connector to reveal a small “text” label. Click the line to type **Manual** or **Automated** (or use the suggestions) and the annotation will render directly on the path—Automated lines automatically turn green. Select a system to expose tiny ✕ handles on its adjacent connections so you can remove links without opening the attached systems.
